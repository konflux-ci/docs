...
. xref:/getting-started/index.adoc#for-developers[Request a new workspace] from your platform engineer. This workspace will serve as the private environment for cloning the affected application. Avoid adding information about the vulnerability to a workspace name.
...
. Log in to the original workspace where the application was previously being developed using the `oc` CLI tool. (See xref:/getting-started/cli.adoc[Getting started with CLI].) Append `/workspaces/<original-tenant-workspace>` to the {ProductName} API URL used in the `oc login` command to target a particular workspace other than your default user workspace.
...
. Log in to the newly created private workspace using the `oc` CLI tool. Append `/workspaces/<private-workspace>` to the {ProductName} API URL used in the `oc login` command to target the new private workspace.
