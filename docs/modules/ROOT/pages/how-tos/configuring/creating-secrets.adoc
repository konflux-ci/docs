...
[IMPORTANT]
==== 
* Secrets lookup mechanism is searching for the most specific secret first. If a secret with a specific repository path is found, it will be used. If not, the secret with the organization path will be used. If no such secret is found as well, the host based secret will be used.
