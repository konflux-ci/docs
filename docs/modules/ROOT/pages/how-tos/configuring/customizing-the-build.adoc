...
== Extending the build pipeline with your own tasks using Trusted Artifacts
...
[NOTE]
====
You can find more tasks link:https://quay.io/organization/konflux-ci[here] and their build definitions link:https://github.com/konflux-ci/build-definitions/tree/main/task[here]. Task generating Trusted Artifacts have `-oci-ta` suffix.
====

.Adding oci-ta task to pipeline 

In your Pipeline definition, add a new task to the `tasks` section.
...
spec:
  tasks:
    ...
    - name: clone-repository-oci-ta
      ...
      - name: ociStorage
        # needs to be unique storage name within the pipeline definition
...
The trusted tasks generate `SOURCE_ARTIFACT` result. This result can be used by other tasks in the pipeline.

.Using `SOURCE_ARTIFACT` in another task

In order to use the `SOURCE_ARTIFACT` in another task, the artifact has to be added as a parameter to the task. The artifact itself is then loaded to a volume accessible only to the task.

You can check link:https://github.com/RedHatInsights/chrome-service-backend/blob/main/.tekton/chrome-service-pull-request.yaml[pipeline] of using the `SOURCE_ARTIFACT` in the pipeline. Look for the `validate-schemas` task.

The task has access to the repository source code, while maintaining the security of the build pipeline and following policies set by EC. This is a common use case for running unit tests, validating the source code, or running a linter.
