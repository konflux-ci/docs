...
For every build, Cachi2 generates a software bill of materials (SBOM) where all dependencies are properly declared and pinned to specific versions. Also, Cachi2 ensures that arbitrary code is never executed during the prefetch, meaning, for example, that the build doesn’t pull any undeclared dependencies. Such measures result in very accurate SBOMs and improve the build reproducibility. For more information about SBOMs, see link:https://konflux-ci.dev/docs/how-tos/metadata/sboms/[Inspecting SBOMs].
...
index server. Your file may look something like this:
...
WARNING: *Do not include credentials in the index url.* If needed, provide authentication via a `.netrc` file (as described below).
...
. In your `.tekton/` PipelineRun files, pass the netrc secret to the pipeline. Add the newly created secret to the `.spec.workspaces` section:
...
. Make sure your PipelineRuns pass the netrc workspace to the prefetch-dependencies task. In the `.spec.pipelineSpec.tasks` section,
  find the entry with `name: prefetch-dependencies`. Add the netrc workspace to the list of workspaces (if not present):
