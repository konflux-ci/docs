...
== Verification
As well as integration test result running for component snapshots, integration test result running for `group` snapshot is also posted back to git pull/merge request via checkRun/commitStatus/comment with pr group name in title.
