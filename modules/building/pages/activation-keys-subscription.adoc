...
[IMPORTANT]
====
The activation key secret is mounted into the build container. Use the following command to register with `subscription-manager`:
[source,bash]
----
subscription-manager register --org "$(cat /activation-key/org)" --activationkey "$(cat /activation-key/activationkey)"
----
The secret will always be mounted in the same directory `/activation-key`, even when custom secret name is used.
====
