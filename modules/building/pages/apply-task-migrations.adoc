...
MintMaker applies the migrations automatically as part of the regular pull requests that update Tekton tasks. However, in case users are required to apply
...
[NOTE]
====
In this context, 'build pipeline' or 'pipeline file' are refering to a YAML file including a
Tekton `Pipeline` or `PipelineRun` definition.
====
