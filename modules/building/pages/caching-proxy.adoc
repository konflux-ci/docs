...
The caching proxy is disabled by default. To enable the caching proxy for a component's build pipeline, set the `enable-cache-proxy` pipeline parameter to `true` and pass it to the `init` task.
...
. Pass the `enable-cache-proxy` parameter to the `init` task in the `tasks` section:
+
[source,yaml]
----
tasks:
