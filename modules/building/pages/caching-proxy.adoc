...
If the caching proxy is not working after setting `enable-cache-proxy` to `true`, the `init` task in your pipeline may not be configured to receive the parameter. This configuration is applied automatically by MintMaker through task migrations.

.Procedure

. Check your repository for any unmerged MintMaker pull requests that update the pipeline configuration.

. If you find an unmerged MintMaker PR that adds the `enable-cache-proxy` parameter to the `init` task, merge it.

. If no MintMaker PR exists, you can manually pass the parameter to the `init` task. In your pipeline definition, add the `enable-cache-proxy` parameter to the `init` task in the `tasks` section:
+
[source,yaml]
----
tasks:
...
. Verify the caching proxy is working by checking the `init` task logs for the proxy configuration messages.
