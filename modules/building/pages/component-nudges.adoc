...
. **Git credentials not configured** - Cannot access target repositories to create PRs
. **Target repository access denied** - Insufficient permissions to create PRs in dependent repositories
...
Check if the nudging finalizer was added to the PipelineRun:

[source,bash]
----
kubectl get pipelinerun <pipelinerun-name> -o yaml | grep finalizer
----

Look for `build.appstudio.openshift.io/build-nudge-finalizer`.

Verify the output image from the nudging component's PipelineRun:
...
Create git credentials for the dependent repositories. See xref:secrets/creating-scm-secrets.adoc[Creating source control management secrets].
