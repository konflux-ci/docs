...
. **build-nudges-ref not configured** - The `spec.build-nudges-ref` field is not set on the nudging Component
...
. **Custom file isn't nudged** - By default files nudged are only `.*Dockerfile.*, .*.yaml, .*Containerfile.*`
. **Wrong trigger for pipeline run** - Only push pipelines triggered by push or incoming (re-run of push pipeline in Konflux UI or by `build.appstudio.openshift.io/request: trigger-pac-build`annotation) event are nudging.
...
. **Custom file isn't nudged**
+
When the files containing image references have different name than `.*Dockerfile.*, .*.yaml, .*Containerfile.*`,
check that they match the patterns specified in the `build.appstudio.openshift.io/build-nudge-files` annotation on nuding push pipeline run (or in custom ConfigMap in `fileMatch`).