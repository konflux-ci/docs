...
Verify the output image from the nudging component's PipelineRun using one of the following:

1. `kubectl` if the `PipelineRun` is still present on the cluster

+
[source,bash]
----
kubectl get pipelinerun <pipelinerun-name> -o jsonpath='{.spec.params[?(@.name=="output-image")].value}'
----

2. `kubectl ka` if the `PipelineRun` has been archived by KubeArchive

+
[source,bash]
----
kubectl ka get pipelinerun <pipelinerun-name> -o jsonpath='{.spec.params[?(@.name=="output-image")].value}'
----

+
See https://kubearchive.github.io/kubearchive/main/cli/usage.html[KubeArchive CLI docs] for more information.
...
Create git credentials for the dependent repositories. Ensure that the provided credentials are
valid, and the respective Kubernetes secret adds the appropriate `appstudio.redhat.com/scm.*`
labels and annotations to specify the correct SCM host and repository.

See xref:secrets/creating-scm-secrets.adoc[Creating source control management secrets].
