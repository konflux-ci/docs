...
** xref:building:creating-forgejo.adoc#creating-a-forgejo-access-token-and-secret[Forgejo access token guide]
...
[NOTE]
====
If you set both the `username` and `password` keys, the authentication type will be considered as `basic`, and a basic authentication client will be created using those credentials.
This client may not work as some Source Code Management (SCM) providers consider basic authentication a deprecated login method.
====
