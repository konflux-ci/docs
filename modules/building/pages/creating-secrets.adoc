...
* **Automatic Secret Linking via UI**
  ...
  (will soon be removed in favor of component specific SA `build-pipeline-$COMPONENT_NAME`).
...
* **Manual Secret Creation**
  . **Link Secret To SA**
    - If you create the secret manually (e.g., via `kubectl` or YAML), you must **manually link it**
      to the `appstudio-pipeline` ServiceAccount in your namespace (will soon be removed in favor of
      component specific SA `build-pipeline-$COMPONENT_NAME`).
