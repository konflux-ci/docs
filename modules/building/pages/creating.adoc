...
`quay.io/redhat-user-workloads/workspace-tenant/`, if you want to use your own repository
...
In quay.io web UI, go to your organization and click on *Create New Repository*, fill out repository name
...
then click on *Create Robot Account*, fill out name of robot account and click on *Create robot account* button,
...
. Enter your Quay.io username in `Username` (from *Robot Accounts* page described above)
. Enter your Quay.io API token in `Password` (from *Robot Accounts* page described above)
. Click on `Add secret`
. Email is optional

This will also add secret to service account `appstudio-pipeline` which will be used for build pipeline.

[IMPORTANT]
====
If you will be also using in your namespace repositories created via ImageRepository, or
different quay.io organization, during creation of the secret when specifying `Registry server address`
you will have to enter full repository name `quay.io/test_organization/own-repository`
====
...
====
If you will be also using in your namespace repositories created via ImageRepository, or
different quay.io organization, you will have to change the repository in the encoded credentials
...
Now we will have to also link the secret to the service account `appstudio-pipeline`,
