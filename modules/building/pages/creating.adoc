...
. Trigger your component's first build pipeline. In the git repository for your component, using your preferred text editor, open a pull request against the `/.tekton/pull-request.yaml` file.
.. Specifically, replace any existing value for the `git-url` field with the git URL for your component's repository. (This is the URL you would use to clone the repository locally; it ends with `.git`.)
