...
Before onboarding a component to {ProductName}, ensure that the instance has appropriate access to the git repository. This means either installing your organization's {ProductName} GitHub App on the source code repository or xref:building:creating-scm-secrets.adoc[creating a secret] to enable access to a GitLab repository.
...
+
NOTE: For *Gitlab* provider, make sure to xref:building:creating-gitlab.adoc#creating-a-gitlab-access-token-and-secret[create a source control secret] before creating the component.
...
{ProductName} supports onboarding components from multiple source control providers using the CLI. Choose the guide for your provider:
...
|GitLab
|xref:building:creating-gitlab.adoc[Onboarding a component from GitLab with the CLI] (requires xref:building:creating-scm-secrets.adoc[source control secrets])
|===
