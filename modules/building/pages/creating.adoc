...
When adding the secret with the UI, it will automatically be linked to the `appstudio-pipeline`
service account so that it can be leveraged by builds.
This will soon change and UI will be allowing linking secret to specific component SA `build-pipeline-$COMPONENT_NAME`,
or to all components within the application.
...
Now we will have to also link the secret to the service account `appstudio-pipeline` (will be removed soon)
or new component specific SA `build-pipeline-$COMPONENT_NAME`,
...
apiVersion: v1
metadata:
  name: appstudio-pipeline / build-pipeline-$COMPONENT_NAME
