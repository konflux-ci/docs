...
Before onboarding a component to {ProductName}, ensure that the instance has appropriate access to the git repository. This means either installing your organization's {ProductName} GitHub App on the source code repository or xref:building:creating-scm-secrets.adoc[creating a secret] to enable access to a GitLab or Forgejo repository.
...
|Forgejo
|xref:building:creating-forgejo.adoc[Onboarding a component from Forgejo with the CLI] (requires xref:building:creating-scm-secrets.adoc[source control secrets])
|===
