...
* Add additional resources to a specific task.
...
Some task steps require resources that scale linearly with their inputs, such as memory, CPU, or
temporary ("ephemeral") filesystem storage. These might exceed the pipeline or system defaults for
your component, causing tasks to fail with truncated logs or error codes greater than 1. To fix
these failures, you will need to increase the resources allocated to the task step.

.Procedure

. Identify which task step is potentially failing due to lack of resources. Consult the `TaskRun`
  logs and error messages to determine this information.

. At the top of the PipelineRun definitions in the `.tekton` directory, set resource requests and limits for
  the step as a `taskRunSpec` override:
...
spec:
  taskRunSpecs:
    - pipelineTaskName: ecosystem-cert-preflight-checks <2>
      stepSpecs:
        - name: app-check <3>
          computeResources: <1>
            requests:
            ...
            limits:
              memory: 4Gi <4>
----
<1> If your `PipelineRun` uses the `v1beta1` API version, the field for specifying compute 
    resources is named is `resources`.
<2> This is the name of the task within the pipeline that is reporting a failure.
<3> This is the name of the step within the task that needs more resources.
<4> Use the same value for `requests` and `limits` to guarantee these resources are allocated to
the task step.

. Repeat for all `PipelineRuns` in the `.tekton` folder which reference this step and
are failing due to lack of memory.

. If your `.tekton` folder has a `Task` object, or a `Pipeline` object with inline `Task`
  definitions, set the resources on the corresponding task step directly:
...
----
+
NOTE: For `Pipeline` objects, setting resources is only supported for inline `taskSpecs`. This is
currently not supported for task references (`taskRef`).
