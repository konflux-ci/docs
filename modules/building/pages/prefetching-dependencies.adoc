...
spec:
    params:
        -   ...
        -   name: prefetch-input
            value: '{"type": "pip", "path": ".", "binary": {"packages": "cryptography", "py_version": 314}}'
----
...
== [[pnpm]]Enabling prefetch builds for `pnpm`

Hermeto supports link:https://pnpm.io[pnpm] by fetching all dependencies in a pnpm-lock.yaml file generated by running the `pnpm install` command.

[IMPORTANT]
====
Refer to the link:https://hermetoproject.github.io/hermeto/latest/pnpm/#example[pnpm hermetic build walkthrough] for more detail on configuration and usage.
...
. Create a key/value secret (see xref:building:secrets/index.adoc[creating secrets for your builds]).
