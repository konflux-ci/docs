...
In {ProductName}, you can run a hermetic build by restricting network access to the build; but without network access, a build will not be able to fetch component dependencies from repote repositories, and might fail. To avoid that, {ProductName} can prefetch dependencies for your hermetic builds using link:https://hermetoproject.github.io/hermeto[Hermeto].
...
IMPORTANT: Refer to link:https://hermetoproject.github.io/hermeto/usage[Hermeto Usage] for more detail on Hermeto configuration and usage. Note, however, that this docs applies specifically to using `hermeto` as a command-line tool, and you might have to make allowances for running in a pipeline in {ProductName}.
...
Refer to the link:https://hermetoproject.github.io/hermeto/bundler/#example[bundler walkthrough] for a detailed example of using bundler with Hermeto.
...
. Create a `artifacts.lock.yaml` file in your git repository, with a list of files to prefetch, their checksums, and optionally their filenames. See the link:https://hermetoproject.github.io/hermeto/generic[Hermeto documentation] for complete overview of the lockfile format.
