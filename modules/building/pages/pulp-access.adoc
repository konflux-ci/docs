...
After the secret is created, mount it in your build pods. The following example demonstrates using it in an RPM build pipeline:
...
The secret includes a `cli.toml` file pre-configured with the domain and mTLS settings. No additional configuration is required.

== Python packages example

The process for Python packages is similar:
