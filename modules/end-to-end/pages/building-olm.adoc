...
[IMPORTANT]
====
The `validate-fbc` task in the `fbc-builder` pipeline enforces that all `olm.bundle` image references in your FBC fragment are pinned by digest (for example, `@sha256:abcdef...`) and come from `registry.redhat.io` or `registry.stage.redhat.io`. Tag-based references or references from other registries cause the build to fail. Pinning by digest ensures immutability, supply-chain integrity, and compatibility with disconnected installations. To keep digests up to date automatically, use xref:building:component-nudges.adoc[component nudges].
====
