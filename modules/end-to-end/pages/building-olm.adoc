...
[IMPORTANT]
====
The `validate-fbc` task in the `fbc-builder` pipeline enforces that all `olm.bundle` image references in your FBC fragment are pinned by digest (for example, `@sha256:abcdef...`). Tag-based references (for example, `:latest` or `:v1.0`) cause the build to fail. Pinning by digest ensures immutability, supply-chain integrity, and compatibility with disconnected installations. To keep digests up to date automatically, use xref:building:component-nudges.adoc[component nudges].
====
