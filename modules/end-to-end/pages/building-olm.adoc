...
[start=2]
. xref:testing:integration/editing.adoc[Edit the Enterprise Contract (EC) ITS]. Note the difference between an ITS and an EC ITS: An ITS can run whatever pipeline you specify. However, an EC ITS is just an ITS that runs a particular pipeline, specifically the Enterprise Contract yaml. https://github.com/konflux-ci/build-definitions/blob/main/pipelines/enterprise-contract.yaml[Here] is an example. 
+

See link:https://github.com/konflux-ci/build-definitions/blob/main/pipelines/enterprise-contract.yaml[for an example] you can modify for your deployment.
