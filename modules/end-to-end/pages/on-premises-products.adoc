...
If you performed a custom installation, you must note all the contexts, namespaces, and other resources you created.
You will need this information to manage Konflux and OpenShift from the command line.
...
- Access to the `crc`, `oc`, and `kubectl` command line tools.
...
To connect to your Konflux UI on the same machine where your Konflux instance is running, open a browser and navigate to https://localhost:9443.
...
To overcome this, you can switch from using local port forwarding (the -L switch) to dynamic port forwarding (the -D switch).
...
This command will create a SOCKS proxy that tunnels all relevant traffic transparently.
You will need to refer to your browser's documentation for instructions on configuring a SOCKS proxy in your browser.

[NOTE]
====
Mozilla Firefox has built-in remote DNS support, but other browsers, like Chrome and Edge, don't, so you'll need to look for a plugin like *Proxy SwitchyOmega* or *FoxyProxy* to handle DNS resolution for you.
====

== Step 2: Create a GitHub Repository For Your Manifest Files

Since a PostgreSQL binary is already available, we'll use it instead of compiling PostgreSQL manually.
To use one of the PostgreSQL binaries, you'll need to create a GitHub repository to hold your Konflux manifest and Tekton pipeline files.
...
Doing so will automatically switch you to the *Applications* tab in the `user-ns2` namespace.
...
From your application's screen, click the Components tab and then select *Add component*.
Complete the *Create a Component* form with the following values:

[cols="1,1"]
|===
|Application name
|postgresql-app (Konflux populates this field for you automatically.)

|Git repository url
|Enter the HTTP link to the GitHub repository you created in Step 2.
...
When finished, click the *Add component* button.
...
Clicking this button will display a page titled *Create release plan*.
...
This namespace is where your release pipeline will execute and where Konflux will deploy your PostgreSQL database server in OpenShift.
...
When enabled, Konflux will automatically attribute the release plan’s author to all releases created from it.
...
. Log in to your OpenShift Console (not Konflux UI).
...
- *Size*: Select a size that suits your needs.
...
You can also view information such as the timestamps when the system triggered each pipeline and the Git commit that triggered the build on this tab.
...
- *Service Integration*: Konflux utilizes the `konflux-integration` namespace to coordinate deployments within your OpenShift cluster.

- *Pipeline as Code: You can view your Tekton pipelines in the `pipelines-as-code` namespace.

- *GitOps*: We recommend storing your products' configurations in a Git repository and letting Konflux manage them through GitOps.

- *Security*: Konflux will run security scans through the `enterprise-contract-service` before deployment.
