...
To see the attestations, you need to find the image for a xref:building:applications-components/creating.adoc#finding-the-built-image[recently completed build pipeline] and export that value to the IMAGE environment variable. Then you can start exploring the information stored in the attestations.
