...
Konflux produces "build-time" and "release-time" SBOMs.

Buld-time SBOMs are created directly during the build phase of the component. 

The "release-time" SBOMs is augmented with additional information which is known only during the release phase of the component. This includes information about the repository to which the component will be released.
...
+
.. Extract the SBOM content from the attestation payload using jq and base64:
...
[source]
----
$ cosign verify-attestation --type spdxjson --key konflux-public-key.pem $IMAGE | jq -r '.payload | @base64d | fromjson | .predicate' > release-sbom.json
----
