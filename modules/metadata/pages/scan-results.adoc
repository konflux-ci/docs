...
* link:https://github.com/csutils/csdiff[The csgrep and csdiff command-line tools can parse SARIF files (available in Fedora/EPEL repos)]

=== csgrep example

[source,shell]
--
❯ csgrep < shellcheck-results.sarif
Error: SHELLCHECK_WARNING:
distgit/containers/rhdh-hub/scripts/rhdh-openshift-setup/quick-start-rhdh.sh:266:30: warning[SC2171]: Found trailing ] outside test. Add missing [ or quote if intentional.
#  264|   # Detect cluster router base if not provided
#  265|   if [[ -z "$ROUTER_BASE" ]]; then
#  266|->   detect_cluster_router_base ]
#  267|       if [ $? -eq 0 ]; then
#  268|         echo "Cluster router base detected: ${ROUTER_BASE}"
--
