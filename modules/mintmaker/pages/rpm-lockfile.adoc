...
The Dockerfile/Containerfile used for generating `rpms.lock.yaml` must have
a base image specified by a registry/name/tag, like so:
...
Using a variable for the base image **is not supported**, for example:

[source,dockerfile]
----
FROM ${BASEIMG} AS baseimg
----
