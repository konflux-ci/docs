...
== Enable automerge for RPM security updates

It's possible to configure RPM security updates to automerge based on the severity of the fixed CVEs. For details on how to enable this feature, see xref:ROOT:mintmaker:user.adoc#automerge-for-rpm-lockfile-updates[Automerge for RPM lockfile updates].
