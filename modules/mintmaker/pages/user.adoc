...
xref:building:secrets/creating-scm-secrets.adoc[setting up the secrets].

NOTE: When updating the GitLab Access Token used by Renovate, the bot's auto-generated user email will change. If any previous Merge Requests (MRs) from the old bot email were not successfully merged, Renovate may incorrectly assume a human is working on that branch and will skip future updates. To prevent this, ensure all existing Renovate Merge Requests are merged or delete any branches created by Renovate where the MR was not successfully merged.
...
The customized `renovate.json` file can be validated with this https://github.com/konflux-ci/renovate-config-validator-action[GitHub Action]. Note that the validator only checks
...
|disable-minor-updates | Disable major and minor semantic version updates
...
|refresh-rpm-lockfiles | Refresh RPM lockfiles after base container image updates
...
CAUTION: Note that the `refresh-rpm-lockfiles` preset disables CVE information in RPM lockfile updates.
...
`droneci`+++</del>+++, `github-actions`, `gitlabci`, `gitlabci-include`, +++<del>+++`jenkins`,
...
|*Infrastructure as Code* |`ansible`, `ansible-galaxy`,
+++<del>+++`bicep`, `crossplane`, `puppet`+++</del>+++, `terraform`, `terragrunt`
...
Enabling automerge for RPM updates involves specific configurations, which are covered in this section. MintMaker can create two types of RPM updates: regular and xref:ROOT:mintmaker:security-updates.adoc[security].
...
NOTE: You can also use one of MintMaker's https://github.com/konflux-ci/mintmaker-presets[prepared presets]. These presets automatically add the `rpmVulnerabilityAutomerge` setting to your config while also enabling automerge for other vulnerability updates.
...
IMPORTANT: Package rules using the https://docs.renovatebot.com/configuration-options/#matchpackagenames[`matchPackageNames`] matcher do not apply to lockfile refreshes. If you want to enable automerge for RPM updates using https://docs.renovatebot.com/configuration-options/#packagerules[`packageRules`], you must create a rule that uses a different matcher (e.g., https://docs.renovatebot.com/configuration-options/#matchmanagers[`matchManagers`] or https://docs.renovatebot.com/configuration-options/#matchupdatetypes[`matchUpdateTypes`]).

NOTE: If you enable general automerge for all RPM updates (by configuring `rpm-lockfile` or `lockFileMaintenance`), the `rpmVulnerabilityAutomerge` setting will be ignored.
...
MintMaker sets `additionalBranchPrefix` to `pass:[{{baseBranch}}/]` by default, which includes
...
* *Before baseBranchPatterns*: `konflux/mintmaker/main/some-package-1.x`
* *After baseBranchPatterns*: `konflux/mintmaker/main-main/some-package-1.x`
