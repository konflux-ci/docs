...
NOTE: For monorepos (multiple components within the same repo), refer to xref:patterns:managing-monorepo-applications.adoc#git-submodules-monorepo[Git submodules in monorepo applications] page.
...
After creating the downstream repository you can onboard the component. See xref:building:applications-components/creating.adoc[Creating applications and components] for more details.
