...
* For any kind of resource specified in the ProjectDevelopmentStreamTemplate, the `creationTimestamp`, `generation`, `resourceVersion`, `uid`, `ownerReferences`, and `finalizers` metadata fields should be removed as well as the `status` section.
