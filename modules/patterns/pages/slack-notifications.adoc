...
[NOTE]
====
* To use `slack-webhook-notification` task, you need to xref:building:secrets/index.adoc[create a secret] in your namespace with at least one key where the value is the webhook URL. For example, to create a secret for Slack, run `kubectl create secret generic my-secret --from-literal dev-team=https://hooks.slack.com/services/XXX/XXXXXX`
