...
<.> specifies the timeout for the entire PipelineRun. Defaults to the global configurable default timeout of 75 minutes.
<.> specifies the timeout for the cumulative time taken by non-finally Tasks specified in the Pipeline. Defaults to the global configurable default timeout of 65 minutes.
...
WARNING: Set the `tasks` timeout to a value lower than the `pipeline` timeout. This ensures there is time for the `finally` tasks to run before the pipeline times out.
