...
Release Pipelines are configured with default CPU and memory values for each Task. These defaults are based on testing with typical workloads and are
...
NOTE: Increasing resources globally for every user would waste cluster resources, so {ProductName} 
supports per Release resource overrides using `taskRunSpecs`.

== Understanding taskRunSpecs
...
== Finding the Pipeline, Task, and Step names
...
Open the Task definition in the repository on the specified branch using the `pathInRepo` value in the `pipeline`.
