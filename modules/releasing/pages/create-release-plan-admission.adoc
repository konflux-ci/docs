...
A ReleasePlanAdmission (RPA) CR exists within a managed workspace. It defines the specific pipeline to run and a given xref:compliance:index.adoc[Enterprise Contract] Policy which needs to pass for the Snapshot before that pipeline can proceed.
