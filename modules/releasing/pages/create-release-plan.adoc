...
=== Snapshots created upon component deletion

When a component is deleted from an application, the integration service automatically creates a new Snapshot containing only the remaining components. These Snapshots have the `release.appstudio.openshift.io/auto-release` label set to `false` to prevent automatic releases, ensuring that component deletions do not trigger unexpected releases.
