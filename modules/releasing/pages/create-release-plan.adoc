...
whether their snapshots are auto-released. Note that `true` value is accepted and means that a *push* Snapshot should be auto-released in all cases unless it is superseded by a newer Snapshot.
...
To avoid promoting a stale build, the integration service by default skips auto-release for that older Snapshot and records that it was superseded. See more about this functionality at the xref:testing:integration/snapshots/snapshot-supersession.adoc[Skipping auto-release for superseded Snapshots] page.
====


{ProductName} also supports custom functions for common use-cases. There is a table of custom
