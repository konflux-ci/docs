...
|roxctl-scan | Scans container images, bundles, operator images for vulnerabilities using roxctl binary, comparing container image against ACS vulnerability database. | Found packages with critical vulnerabilities associated with RHSA fixes. Only for internal usage, not for community builds.
|===
