...
+
NOTE: Starting an integration test is dependant on the results of a successful build pipeline providing some required information about an image, details on these requirements can be found link:https://github.com/konflux-ci/architecture/blob/main/architecture/core/integration-service.md#image-extraction-details[here]. In the case of multi-arch builds, an OCI index that maps to the relevant artifacts is used.
