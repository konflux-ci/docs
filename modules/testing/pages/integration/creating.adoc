...
. In the `.pipeline.spec` path, declare a new task that list all containerImage and git source for all components in snapshot. More tests can be added to test the containerImage and git source.
      ...
      - name: test-snapshot-component
        ...
        script: |
          ...
          do
            ...
            echo "Print the infomation for component ${componentName} included in snapshot, more test can be added to test this component:"
            echo "${componentName}"
            echo "${componentUrl}"
