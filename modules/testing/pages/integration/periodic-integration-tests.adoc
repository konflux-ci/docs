...
This procedure details how to grant the necessary snapshot access to a ServiceAccount, which is required to run periodic Integration Tests. It assumes you have an integration test created and the `periodic-jobs-runner` ServiceAccount available in your tenant namespace.
    ...
    spec:
      template:
        spec:
          ...
          restartPolicy: Never <.>
          serviceAccountName: periodic-jobs-runner
          serviceAccount: periodic-jobs-runner
----
