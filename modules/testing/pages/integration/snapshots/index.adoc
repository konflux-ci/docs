...
* **Automatic (Component Deletion):** When a component is deleted from an application, the integration service automatically creates a new Snapshot containing only the remaining components. This Snapshot has the `release.appstudio.openshift.io/auto-release` label set to `false`, so it will not be automatically released even if your ReleasePlan is configured for auto-release. This ensures that component deletions do not trigger unexpected releases.
...
[IMPORTANT]
====
**Protect Snapshots from Garbage Collection**

**test.appstudio.openshift.io/keep-snapshot**

The annotation supports two formats:

**true**: Snapshots annotated with `test.appstudio.openshift.io/keep-snapshot=true` will **not** be garbage collected, regardless of retention counts. This annotation acts as a safety mechanism to protect critical Snapshots from deletion. Apply this annotation to Snapshots that you need to preserve for extended periods

**TTL Format Duration strings**: Protects the Snapshot for a specific duration using Go duration format (e.g., ` 2h9m13s`). The Snapshot will be automatically deleted after the TTL expires, calculated from the Snapshot's creation timestamp. 

**Important Notes:** If the duration format is invalid or cannot be parsed, the Snapshot will be treated as unprotected and may be garbage collected. Protected Snapshots still count towards the retention count quota, so use this annotation judiciously.
