...
Snapshots annotated with `test.appstudio.openshift.io/keep-snapshot=true` will **not** be garbage collected, regardless of retention counts. This annotation acts as a safety mechanism to protect critical Snapshots from deletion. Apply this annotation to Snapshots that you need to preserve for extended periods.

**Note:** Protected Snapshots still count towards the retention count quota, so use this annotation judiciously.
