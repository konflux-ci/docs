...
This one is located within build definition task roxctl scan. SCAN_OUTPUT result[json] is displayed in different manner than TEST_OUTPUT, but it serves the same purpose.
It additionaly displays found vulnerabilities, patched(RHSA fix) and unpatched ones. Should be used in addition to TEST_OUTPUT.

image::vulnerabilities.png[role="border" alt="Vulnerabilities"]

NOTE: Scan results within Konflux will only report vulnerabilities associated with RHSAs as fixable. This can lead to inconsistencies with Roxctl scan results found in Quay or Clair or elsewhere.
